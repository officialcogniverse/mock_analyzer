...
import { getOrCreateUserId } from "@/lib/userClient";
...
export default function HistoryDashboard() {
  ...
  useEffect(() => {
    try {
      ...
      toast.error("Could not access local storage. Try another browser.");
    }
  }, []);
  ...
  async function loadAll(uid: string) {
    ...
    try {
      ...
      const insRes = await fetch(
        `/api/insights?userId=${encodeURIComponent(uid)}&exam=${encodeURIComponent(
          ex
        )}&lastN=10`
      );
