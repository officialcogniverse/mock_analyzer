...
import { ModeTabs } from "@/components/landing/ModeTabs";
import type {
  AnalysisResult,
  ChatMode,
  IntakeAnswers,
...
const quickChipsByMode: Record<ChatMode, string[]> = {
  BREAKDOWN: ["Where did I lose marks?", "Explain the top errors", "Show mistake patterns"],
  STRATEGY: ["Give me a 48-hour plan", "How to boost accuracy?", "What to prioritize next?"],
  MOTIVATION: ["I feel stuck", "Help me stay consistent", "Give me a reset pep talk"],
};

const modeGreeting: Record<ChatMode, string> = {
  BREAKDOWN: "Let’s break down your mock and pinpoint what matters most.",
  STRATEGY: "Strategy mode: we’ll convert insights into the next 7-14 days.",
  MOTIVATION: "Motivation mode: supportive, practical guidance to keep you moving.",
};

export function IntakeBot({ intake, onIntakeChange, analysis, onReset }: IntakeBotProps) {
  const [mode, setMode] = useState<ChatMode>("BREAKDOWN");
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      ...
      if (payload.response?.disclaimer) {
        setDisclaimer(payload.response.disclaimer);
      }
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: payload.response?.reply ?? "I’m here to help—try again with a shorter prompt.",
        },
      ]);
    } catch (err) {
    ...
    <aside className="surface-card flex h-full flex-col gap-6 p-6">
      ...
      {!analysis ? (
        <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              {examOptions.map((option) => (
                <Button
                  ...
                  onClick={() => updateIntake({ exam: option.id })}
                >
                  {option.label}
                </Button>
              ))}
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(event) => {
                ...
                updateIntake({ daysToExam: value ? Number(value) : undefined });
              }}
              placeholder="e.g. 45"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {situationOptions.map((option) => (
                <Button
                  ...
                  className="rounded-full"
                  onClick={() => updateIntake({ situation: option.id })}
                >
                  {option.label}
                </Button>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              {weeklyOptions.map((option) => (
                <Button
                  ...
                  className="rounded-full"
                  onClick={() => updateIntake({ weeklyHours: option.id })}
                >
                  {option.label}
                </Button>
              ))}
            ...
            <div className="flex flex-wrap gap-2">
              {routineOptions.map((option) => (
                <Button
                  ...
                  className="rounded-full"
                  onClick={() => updateIntake({ routineType: option.id })}
                >
                  {option.label}
                </Button>
              ))}
          ...
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(event) =>
                updateIntake({
                  biggestPainPoint: event.target.value ? event.target.value : undefined,
                })
              }
        ...
        <div className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">{modeGreeting[mode]}</p>

          <div className="flex flex-wrap gap-2">
            {quickChipsByMode[mode].map((chip) => (
              <button
                key={chip}
                type="button"
